...
import { getStatusDisplayLabel, getStatusLegendLabel, getStatusTooltip, type DischargeStatus } from "@/lib/utils/status-labels";
...
export function DischargeScore({ score, status, isLoading = false }: DischargeScoreProps) {
  ...
  const getStatusText = () => {
    return getStatusDisplayLabel(status as DischargeStatus);
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8">
      ...
      {!isLoading && (
        ...
        >
          <Tooltip content={getStatusTooltip("not_ready")} position="bottom">
            <div className="flex items-center gap-2 cursor-help">
              <span className="w-3 h-3 rounded-full bg-red-500" />
              <span>0-39: {getStatusLegendLabel("not_ready")}</span>
            </div>
          </Tooltip>
          <Tooltip content={getStatusTooltip("caution")} position="bottom">
            <div className="flex items-center gap-2 cursor-help">
              <span className="w-3 h-3 rounded-full bg-amber-500" />
              <span>40-69: {getStatusLegendLabel("caution")}</span>
            </div>
          </Tooltip>
          <Tooltip content={getStatusTooltip("ready")} position="bottom">
            <div className="flex items-center gap-2 cursor-help">
              <span className="w-3 h-3 rounded-full bg-emerald-500" />
              <span>70-100: {getStatusLegendLabel("ready")}</span>
            </div>
          </Tooltip>
        </motion.div>
      )}
            ...
            {showMethodology && (
              ...
              >
                <div className="mt-3 bg-gray-50 rounded-lg p-4 text-xs text-gray-600 space-y-2">
                  ...
                  <p className="text-gray-400 pt-1">Score is generated by AI analysis weighing the severity and clinical significance of identified risk factors. Use as decision support alongside clinical judgment.</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
